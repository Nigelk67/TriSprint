...
class ActivityViewModel_Tests: XCTestCase {
    ...
    func test_formatDisplayPacePerKm_shouldShowPaceString() {
        ...
        for _ in 0..<loopCount {
            ...
            let paceString = String(format: "%.2f",paceInKm)
    ...
    func test_formatDisplayPacePerMile_shouldShowPaceString() {
        ...
        for _ in 0..<loopCount {
            ...
            let paceString = String(format: "%.2f", paceInMi)
    ...
    func test_formatDisplayKmph_shouldShowKmSpeedString() {
        ...
        for _ in 0..<loopCount {
            ...
            let speedString = String(format: "%.2f", speedInKm)
    ...
    func test_formatDisplayMph_shouldShowMphSpeedString() {
        let loopCount: Int = Int.random(in: 1..<20)
        var formattedSpeed = ""
        var trimmedString = ""
        for _ in 0..<loopCount {
            ...
            let seconds = Double.random(in: 1000..<5000)
            let hour = seconds / 3600
            let speedInMi = miles / hour
            let speedString = String(format: "%.2f", speedInMi)
            trimmedString = speedString + " mi/hr"
            formattedSpeed = FormatDisplay.speedMph(distance: mtrsRandom, seconds: seconds)
        }
        XCTAssertEqual(trimmedString, formattedSpeed)
    }
